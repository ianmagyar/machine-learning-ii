...
def optimize_model(state, action, next_state, reward):
    ...
    transitions = [Transition(state, action, next_state, reward)]
    ...
    non_finals = [s for s in batch.next_state if s is not None]
    if non_finals:
        non_final_next_states = torch.cat(non_finals)
        with torch.no_grad():
